...
# Takes a tensor A and zeros 2 of every 4 elements.
def make_sparse(A):
...
# Python implementation of compression - removing zeros from the A matrix,
# and creating the metadata matrix.
# Format:
#   an [M, K] matrix A is converted to a sparse [M, K/2] amatrix
#       and a [M, K//16] metadata matrix aMeta.
...
def get_autotune_config():
    return [
        ...
        triton.Config({'BLOCK_SIZE_M': 128, 'BLOCK_SIZE_N': 128, 'BLOCK_SIZE_K': 128, 'GROUP_SIZE_M': 8}, num_warps=8, num_stages=4),
        triton.Config({'BLOCK_SIZE_M': 128, 'BLOCK_SIZE_N': 128, 'BLOCK_SIZE_K': 128, 'GROUP_SIZE_M': 8}, num_warps=4, num_stages=3),
        triton.Config({'BLOCK_SIZE_M': 128, 'BLOCK_SIZE_N': 128, 'BLOCK_SIZE_K': 128, 'GROUP_SIZE_M': 8}, num_warps=4, num_stages=4),
        triton.Config({'BLOCK_SIZE_M': 256, 'BLOCK_SIZE_N': 128, 'BLOCK_SIZE_K': 128, 'GROUP_SIZE_M': 8}, num_warps=8, num_stages=3),
        triton.Config({'BLOCK_SIZE_M': 256, 'BLOCK_SIZE_N': 128, 'BLOCK_SIZE_K': 128, 'GROUP_SIZE_M': 8}, num_warps=8, num_stages=3),
        triton.Config({'BLOCK_SIZE_M': 128, 'BLOCK_SIZE_N': 256, 'BLOCK_SIZE_K': 128, 'GROUP_SIZE_M': 8}, num_warps=8, num_stages=3),
        triton.Config({'BLOCK_SIZE_M': 128, 'BLOCK_SIZE_N': 128, 'BLOCK_SIZE_K': 256, 'GROUP_SIZE_M': 8}, num_warps=8, num_stages=3),
        triton.Config({'BLOCK_SIZE_M': 256, 'BLOCK_SIZE_N': 128, 'BLOCK_SIZE_K': 128, 'GROUP_SIZE_M': 16}, num_warps=8, num_stages=4),
        triton.Config({'BLOCK_SIZE_M': 128, 'BLOCK_SIZE_N': 256, 'BLOCK_SIZE_K': 128, 'GROUP_SIZE_M': 16}, num_warps=8, num_stages=4),
        triton.Config({'BLOCK_SIZE_M': 128, 'BLOCK_SIZE_N': 128, 'BLOCK_SIZE_K': 256, 'GROUP_SIZE_M': 16}, num_warps=8, num_stages=4),
    ]
    return configs
...
def matmul_kernel_sparse(
    ...
    for k in range(0, K_LIMIT):
        ...
        a = tl.load(a_ptrs, mask=offs_k_a[None, :] < K // 2, other=0.0)
        ...
        # @nocommit.
        b = tl.load(b_ptrs, mask=offs_k_b[:, None] < K, other=0.0)
        aMeta = tl.load(
            aMeta_ptrs,
            mask=offs_k_aMeta[None, :] < K // 16,
            other=0,
        )
        ...
        offs_k_a += BLOCK_SIZE_K_A
        offs_k_b += BLOCK_SIZE_K
        offs_k_aMeta += BLOCK_SIZE_K_A_META
...
def matmul(aSparse, aMeta, b, out_dtype):
    # Check constraints.
    ...
    # Allocates output.
    c = torch.empty((M, N), device=aSparse.device, dtype=out_dtype)
...
def do_bench_repeat(fn):
    # this benchmarking setup yields more stable results
    REPEAT = 5
    ITERATIONS = 11
    results = []
...
def check_accuracy(a, aSparse, aMeta, b, out_dtype, tolerance):
    if a.dtype.itemsize == 1:
        one_device = torch.tensor(1., device=a.device)
        torch_output = torch._scaled_mm(a, b, scale_a=one_device, scale_b=one_device, out_dtype=out_dtype, use_fast_accum=True)
    else:
        torch_output = torch.matmul(a, b)
    triton_output = matmul(aSparse, aMeta, b, out_dtype=out_dtype)
    if torch.allclose(triton_output, torch_output, atol=tolerance, rtol=tolerance):
        print(f"  Correctness {a.dtype}, tolerance {tolerance}: Triton and Torch match! ✅")
    else:
        print(f"  Correctness {a.dtype}, tolerance {tolerance}: Triton and Torch differ! ❌")


def benchmark_triton(aSparse, aMeta, b, out_dtype):
    ms = do_bench_repeat(lambda: matmul(aSparse, aMeta, b, out_dtype))
...
def test_inputs(a, aSparse, aMeta, b, out_dtype, tolerance):
...
test_dim = 8192
a, aSparse, aMeta, b = get_inputs(test_dim, test_dim, test_dim)

print("FP16 results:")
assert a.dtype == torch.float16
test_inputs(a, aSparse, aMeta, b, torch.float16, 1e-3)

print("BF16 results:")
assert a.dtype == torch.float16
test_inputs(a.to(torch.bfloat16), aSparse.to(torch.bfloat16), aMeta, b.to(torch.bfloat16), torch.bfloat16, 1e-2)

print("FP8 E4M3 results:")
assert a.dtype == torch.float16
test_inputs(a.to(torch.float8_e4m3fn), aSparse.to(torch.float8_e4m3fn), aMeta, b.to(torch.float8_e4m3fn).T, torch.float16, 0.4)
